# frozen_string_literal: true

#
# Generated by serializers-code-gen
# Descriptor: RecursiveMutualItemSerializer
# Mode: :json
# Config: null_for_missing_has_one=true, supports_root_key=false, hash_record_key_type=:string, hash_output_key_type=:string, json_column_emit=:wire_format, pool_writer=true, guarded_model=false
#
# To use:
#   require_relative "recursive_mutual_item_serializer_json.generated"
#   RecursiveMutualItemSerializer_JSON.new(descriptor: descriptor).serialize_one(record)

require_relative "recursive_mutual_folder_serializer_json"

class RecursiveMutualItemSerializer_JSON
  FIELD_INDEX = {id: 0, name: 1, subfolder: 2}.freeze
  POOL = Panko::CodeGen::WritersPool::IsolatedExecutionState.new(:_scg_writer__RecursiveMutualItemSerializer_JSON)

  def initialize(descriptor:, _construct_cache: {})
    _construct_cache[descriptor.__id__] = self
    @subfolder_serializer = (_construct_cache[descriptor.associations[0].descriptor.__id__] ||= RecursiveMutualFolderSerializer_JSON.new(descriptor: descriptor.associations[0].descriptor, _construct_cache: _construct_cache))
  end

  def serialize_one(record, context: nil, scope: nil, filters: nil)
    filters = Panko::CodeGen::Filter.wrap(filters, FIELD_INDEX)
    writer = POOL.checkout
    begin
      _write_one(record, writer, context, scope, filters)
      result = writer.to_s
      result.chomp!
      result
    ensure
      POOL.checkin(writer)
    end
  end

  def serialize_many(records, context: nil, scope: nil, filters: nil)
    filters = Panko::CodeGen::Filter.wrap(filters, FIELD_INDEX)
    writer = POOL.checkout
    begin
      writer.push_array
      records.each { |r| _write_one(r, writer, context, scope, filters) }
      writer.pop
      result = writer.to_s
      result.chomp!
      result
    ensure
      POOL.checkin(writer)
    end
  end

  def _release
    nil
  end

  def _write_one(record, writer, context, scope, filters)
    if record.is_a?(Hash)
      writer.push_object
      unless filters.drops?(0)
        writer.push_value(record["id"], "id")
      end
      unless filters.drops?(1)
        writer.push_value(record["name"], "name")
      end
      unless filters.drops?(2)
        value = record["subfolder"]
        if value.nil?
          writer.push_value(nil, "subfolder")
        else
          writer.push_key("subfolder")
          @subfolder_serializer._write_one(value, writer, context, scope, filters.child(:subfolder, RecursiveMutualFolderSerializer_JSON::FIELD_INDEX))
        end
      end
      writer.pop
    else
      writer.push_object
      unless filters.drops?(0)
        writer.push_value(record.id, "id")
      end
      unless filters.drops?(1)
        writer.push_value(record.name, "name")
      end
      unless filters.drops?(2)
        value = record.subfolder
        if value.nil?
          writer.push_value(nil, "subfolder")
        else
          writer.push_key("subfolder")
          @subfolder_serializer._write_one(value, writer, context, scope, filters.child(:subfolder, RecursiveMutualFolderSerializer_JSON::FIELD_INDEX))
        end
      end
      writer.pop
    end
  end
end
