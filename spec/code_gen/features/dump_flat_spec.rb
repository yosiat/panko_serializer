...
RSpec.describe "Panko::CodeGen.dump (flat single-file)" do
    ...
    [Fixtures::ShallowGeneric, Fixtures::ShallowSpecialized].each do |fixture|
      ...
      fixture::MODES.each do |mode|
        it "matches the snapshot for #{fixture.name} in #{mode} Output Mode" do
          ...
          snapshot_path = File.join(Panko::CodeGen::Spec::SNAPSHOTS_DIR, snapshot_filename)
  ...
  describe "path: validation" do
    it "raises ArgumentError when path: is nil" do
      expect {
        Panko::CodeGen.dump(descriptor, output: :json, config: config, path: nil)
      }.to raise_error(ArgumentError, /path:/)
    end

    it "raises ArgumentError when path: is an empty String" do
      expect {
        Panko::CodeGen.dump(descriptor, output: :json, config: config, path: "")
      }.to raise_error(ArgumentError, /path:/)
    end

    it "raises ArgumentError when path: is a Symbol" do
      expect {
        Panko::CodeGen.dump(descriptor, output: :json, config: config, path: :foo)
      }.to raise_error(ArgumentError, /path:/)
    end

    it "raises ArgumentError when path: is an Integer" do
      expect {
        Panko::CodeGen.dump(descriptor, output: :json, config: config, path: 42)
      }.to raise_error(ArgumentError, /path:/)
    end

    it "does not run the Validator when path: is invalid" do
      validator = instance_spy(Panko::CodeGen::Validators::Validator)
      dump = Panko::CodeGen::Dump.new(
    ...
    it "does not run the Generator when path: is invalid" do
      generator = instance_spy(Panko::CodeGen::Generator)
      dump = Panko::CodeGen::Dump.new(
    ...
    it "writes no file when path: is invalid (no partial-write side effect)" do
      Dir.mktmpdir do |dir|
        [nil, "", :symbol, 42].each do |bogus|
          expect {
            Panko::CodeGen.dump(descriptor, output: :json, config: config, path: bogus)
