...
| **Sink**              | The Output Mode seam inside the **Generator**: the interface every mode-divergent emit shape sits behind. `JsonSink` and `HashSink` are its two adapters; the shared emitters (`ClassEmitter`, `FieldWalk`, the record-access strategies) talk only to it. Distinct from **Writer** (a runtime object inside the generated code). | Backend, target, emitter (for the adapters) |
...
| **Writer**            | An `Oj::StringWriter` instance allocated at the top of a JSON-mode serialization and threaded through composed Generated Classes. | Stream, buffer                   |
...
- **"Field"** is the **union** of **Attribute**, **Method Attribute**, and **Association** — not a synonym for **Attribute**. When someone says "filter this field," they may mean any of the three. When you need to be specific about the source-of-value (direct read vs callable vs nested serializer), use the specific kind name. When the distinction doesn't matter (e.g., "every Field is filterable"), **Field** is the right word.
