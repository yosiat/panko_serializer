...
class PostSerializer_JSON
  ...
  def _write_one(record, writer, context, scope, filters)
    ...
    if @cb_if_comments.call(record, context)
      ...
      @comments_serializer._write_one(record.public_comments, writer, context, scope, filters)
    end
    writer.pop
  end
...
The `# Generated by Panko::CodeGen` header is the literal banner emitted into every
dumped file — it is stamped from `GENERATOR_NAME` in
[`version.rb`](../../lib/panko/code_gen/version.rb).
...
```ruby
Panko::CodeGen.dump(descriptor, output: :json, config: Config.new, path: "app/generated/post_serializer_json.rb")
```
...
A CLI (`$ panko dump ...`) is out of scope for v1 — callers drive the
facade from Ruby.
...
- **Compile** passes the synthetic identifier `"(Panko::CodeGen: <Name>/<output>)"`
