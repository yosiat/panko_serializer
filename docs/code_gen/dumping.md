...
The `# Generated by serializers-code-gen` header is the literal banner emitted into
every dumped file — it is stamped from `GENERATOR_NAME` in
[`version.rb`](../../lib/panko/code_gen/version.rb), which still reads
`"serializers-code-gen"`, so the on-disk text keeps that name even though the module is
`Panko::CodeGen`.
